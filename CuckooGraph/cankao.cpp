...
void LoadDataSparseDense(std::string path)
{
    ...
    ifstream infile;
    infile.open(path); // 稀疏图
    // infile.open("/root/Cuckoo/0126kddbeifen1/CuckooGraphDatasetNewNew.txt");//密集图
    if (!infile.is_open())
    {
...
void LoadDataNotredame(std::string path)
{
    ...
    infile.getline(useless, intbig); // notredame
    infile.getline(useless, intbig); // notredame
    infile.getline(useless, intbig); // notredame
    ...
    while (1)
    {
        ...
        asrc.push_back(stoi(src));
        adst.push_back(stoi(dst));
        //      asrc.push_back(stoi(src)+1);//notredame
        //      adst.push_back(stoi(dst)+1);//notredame
        maxVertex = std::max(asrc[i], maxVertex);
        maxVertex = std::max(adst[i], maxVertex);
        vertex.insert(asrc[i]);
        vertex.insert(adst[i]);
