...
Auxo::Auxo(int width, int range, int p_num, int f_num)
{
	...
	p = p_num;
	f = f_num;
	rfnum = 2 * ceil(log(r) / log(2));	halfR = rfnum / 2;
	rper = 64 / rfnum;	matrixSize = w * w;
...
bool Auxo::insert(string s1, string s2, int weight) {
...
int Auxo::edgeQuery(string s1, string s2) {
	int res = 0;
	ul hash1 = mpq->MPQHash(s1.c_str(), Type);
	ul hash2 = mpq->MPQHash(s2.c_str(), Type);
	unsigned int g1 = hash1 & fvalue[f];
...
int Auxo::nodeValueQuery(string s1, int type, double& queryMatrix) {
	int weight = 0;
	ul hash1 = mpq->MPQHash(s1.c_str(), Type);
	unsigned int g1 = hash1 & fvalue[f];
	if (g1 == 0) g1 += 1;
