...
public:
	...
	int n;
	int edge_num; 		GSS(int width, int range, int p_num, int size, int f_num, bool usetable, int tablesize = 0); 		~GSS()
	{
		delete[] value;
		cleanupBuffer();
	...
	int nodeValueQuery(string s1, int type);		 int nodeDegreeQuery(string s1, int type);		 void nodeSuccessorQuery(string s1, vector<string>& IDs);
...
void GSS::nodeSuccessorQuery(string s1, vector<string>& IDs) {
	...
	if (it != index.end())
	{
		...
		linknode* node = buffer[tag];
		if (node->weight != 0)
			mapTable.getID(k1, IDs); 			node = node->next;
		while (node != NULL)
		{
...
int GSS::nodeDegreeQuery(string s1, int type) {
	...
	if (type == 0)
	{
		...
		if (it != index.end())
		{
			...
			linknode* node = buffer[tag];
			if (node->weight != 0);
			degree += mapTable.countIDnums(k1); 			node = node->next;
			while (node != NULL)
			{
	...
	{
		unsigned int k1 = (h1 << f) + g1;
		map<unsigned int, int>::iterator it = index.find(k1); 		if (it != index.end())
		{
			if (buffer[it->second]->weight != 0)
