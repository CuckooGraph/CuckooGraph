...
public:
	...
	count_type query_edge(chain* edge_q);
	count_type query_edge_ins(chain* edge_q);
...
void Chandelier::insert(chain* edge_new, int num = 0) {
	chain* temp;
	count_type ans = query_edge_ins(edge_new);
	unsigned int ra[raw];
	unsigned int co[col];
	int cnt[raw * col];
	if (ans == 0) {
...
count_type Chandelier::query_edge(chain* edge_q) {
	unsigned int ra[raw];
	unsigned int co[col];
	for (int i = 0; i < raw; ++i) {
...
count_type Chandelier::query_edge_ins(chain* edge_q) {
	unsigned int ra[raw];
	unsigned int co[col];
	for (int i = 0; i < raw; ++i) {
		ra[i] = (mmhash(edge_q->src, rawseed[i])) % len;
