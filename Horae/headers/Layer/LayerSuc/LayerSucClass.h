...
public:
	...
	weight_type nodeQuery(string vertex, int type);			bool reachabilityQuery(string s, string d);
};
...
void LayerSucClass::insert(string src, string dst, weight_type weight) {
	...
	if (!inserted) {
			...
			if (it != successorAdjacencyList.end()) {							vector<node>::iterator iter = find_if(it->begin(), it->end(), findx(k2));
				if (iter != it->end()) {						iter->weight += weight;
					return;
																																							}
			}
			if (!cache_align) {							inserted = kickElement(addr_src, fp_src, addr_dst, fp_dst, weight);
			}
			else {
...
weight_type LayerSucClass::nodeQuery(string vertex, int type) { 	weight_type weight = 0;
...
bool LayerSucClass::reachabilityQuery(string s, string d) {
	...
	if (cache_align) 
	{
		while (!q.empty())
		{
			...
			{
				tmp1[i] = (tmp1[i - 1] * ultiplier + ncrement) % odulus;
			}
			for (int i = 1; i < column_addrs / 2; i++) 
			{
				tmp2[i] = (tmp2[i - 1] * ultiplier + ncrement) % odulus;
			}
					...
					for (int i3 = 0; i3 < SLOTNUM; i3++)
					{
						if ((value[pos].src[i3] & mask) == temp_fp && ((value[pos].src[i3] >> 14) == i1))
						{
							...
							for (int v = 0; v < tmp_s; v++)
								shifter = (shifter * ultiplier + ncrement) % odulus;
							uint32_t tmp_h = i2;
							while (tmp_h < shifter)
								tmp_h += width;											tmp_h -= shifter;
