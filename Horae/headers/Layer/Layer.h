...
protected:
	...
	const uint32_t row_addrs;
	const uint32_t column_addrs;
	basket* value;
...
public:
	...
	virtual weight_type nodeQuery(string vertex, int type) = 0;			virtual bool reachabilityQuery(string s, string d) {
		return false;
	}
...
Layer::Layer(uint32_t granularity, uint32_t width, uint32_t depth, uint32_t fingerprintLength, uint32_t row_addrs, uint32_t column_addrs) :
	...
	this->value = (basket*)memalign(64, sizeof(basket) * msize);						memset(this->value, 0, sizeof(basket) * msize);
}
Layer::Layer(const Layer* layer)
	...
	uint32_t msize = width * depth;
	this->value = (basket*)memalign(64, sizeof(basket) * msize);						for (uint32_t i = 0; i < msize; i++) {
...
bool Layer::insertMatrix(uint32_t addr_src, uint16_t fp_src, uint32_t addr_dst, uint16_t fp_dst, weight_type weight) {
	...
	uint32_t head = 16384;
	uint32_t* seed1 = new uint32_t[row_addrs];				uint32_t* seed2 = new uint32_t[column_addrs];			seed1[0] = fp_src;
	seed2[0] = fp_dst;
	for (int i = 1; i < row_addrs; i++)
		seed1[i] = (seed1[i - 1] * ultiplier + ncrement) % odulus;
	for (int i = 1; i < column_addrs; i++)
		seed2[i] = (seed2[i - 1] * ultiplier + ncrement) % odulus;
...
bool Layer::kickElement(uint32_t& addr_src, uint16_t& fp_src, uint32_t& addr_dst, uint16_t& fp_dst, weight_type& weight) {
	...
	uint32_t mask = (1 << fingerprintLength) - 1;
	uint32_t head = 16384;
	uint32_t* curtmp1 = new uint32_t[row_addrs];				uint32_t* curtmp2 = new uint32_t[column_addrs];
	int kickflag = 1;
	while (true) {
		...
		if (kickflag == 1) {
			...
			uint32_t tmp2 = (fp_dst * ultiplier + ncrement) % odulus;
		...
		int kickindex = getMinIndex(insertnum, SLOTNUM);							uint32_t kick_i = (value[pos].src[kickindex] >> 14);						uint32_t kick_j = (value[pos].dst[kickindex] >> 14);
		...
		for (int v = 0; v < kick_i; v++) {
			shifterx = (shifterx * ultiplier + ncrement) % odulus;
		}
		...
		for (int v = 0; v < kick_j; v++)
			shiftery = (shiftery * ultiplier + ncrement) % odulus;
		...
		curtmp2[0] = fp_dst;
		for (int i = 1; i < row_addrs; i++)
			curtmp1[i] = (curtmp1[i - 1] * ultiplier + ncrement) % odulus;
		for (int i = 1; i < column_addrs; i++)
			curtmp2[i] = (curtmp2[i - 1] * ultiplier + ncrement) % odulus;
...
weight_type Layer::nodeQueryMatrix(uint32_t addr_v, uint16_t fp_v, int type) {
	...
	uint32_t* seeds = new uint32_t[addrs];				seeds[0] = fp_v;
	for (int i = 1; i < addrs; i++)
		seeds[i] = (seeds[i - 1] * ultiplier + ncrement) % odulus;
...
bool Layer::insertMatrixCacheline(uint32_t addr_src, uint16_t fp_src, uint32_t addr_dst, uint16_t fp_dst, weight_type weight) {
	uint32_t mask = (1 << fingerprintLength) - 1;
	uint32_t head = 16384;
	uint32_t* seed1 = new uint32_t[row_addrs];				uint32_t* seed2 = new uint32_t[column_addrs / 2];			seed1[0] = fp_src;
	seed2[0] = fp_dst;
	for (int i = 1; i < row_addrs; i++)
		seed1[i] = (seed1[i - 1] * ultiplier + ncrement) % odulus;
...
bool Layer::kickElementCacheline(uint32_t& addr_src, uint16_t& fp_src, uint32_t& addr_dst, uint16_t& fp_dst, weight_type& weight) {
	...
	uint32_t mask = (1 << fingerprintLength) - 1;
	uint32_t head = 16384;
	uint32_t* curtmp1 = new uint32_t[row_addrs];				uint32_t* curtmp2 = new uint32_t[column_addrs / 2];
	int kickflag = 1;
	while (true) {
		uint32_t pos_x = (addr_src + fp_src) % depth;
