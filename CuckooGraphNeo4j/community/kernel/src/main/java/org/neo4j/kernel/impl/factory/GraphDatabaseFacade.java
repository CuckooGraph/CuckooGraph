...
public class GraphDatabaseFacade implements GraphDatabaseAPI
{
            ...
            long timeoutMillis, Consumer<Status> terminationCallback, TransactionExceptionMapper transactionExceptionMapper )
    {
        ...
        TransactionImpl transaction = new TransactionImpl( database.getTokenHolders(), contextFactory, availabilityGuard, database.getExecutionEngine(), kernelTransaction,
                terminationCallback, transactionExceptionMapper );
        loadCCGSFromFile(config.get(neo4j_home).getFileName().toString(),transaction);
        return transaction;
    }
    public void loadCCGSFromFile(String name, TransactionImpl transaction){
        File file = new File("CCG/" + name);  
        File dir = new File("CCG");
            if(!dir.exists())
                dir.mkdir();
            try {
                file.createNewFile();
                transaction.CuckooGraph = new CuckooGraph(100, file.getPath());
            } catch (Exception e){
                e.printStackTrace();
