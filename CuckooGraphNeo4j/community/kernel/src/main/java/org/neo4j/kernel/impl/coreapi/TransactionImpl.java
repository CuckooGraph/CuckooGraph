...
public class TransactionImpl implements InternalTransaction
{
    ...
    public CuckooGraph CuckooGraph;
    ...
    public void commit()
    {
        ...
        File file = new File(CuckooGraph.filePath);
        CuckooGraph.SaveToFile(file, CuckooGraph);
    }
    ...
    @Override
    public Iterator<Relationship> getRelationships(int srcNodeID, int dstNodeID){
        org.neo4j.graphdb.CuckooGraph.CGRelation ccgRelation = CuckooGraph.queryRel(srcNodeID, dstNodeID);
        return new CCGIterator(ccgRelation, CuckooGraph);
    }

    public class CCGIterator implements ResourceIterator<Relationship>
    {
        public ArrayList<Integer> array;
        public int pos;
        public int num;

        public CCGIterator(org.neo4j.graphdb.CuckooGraph.CGRelation target, CuckooGraph sketch)
        {
            ...
            if(target != null) {
                int tool = target.nextRec;
                while (tool != -1) {
                    array.add(sketch.records.get(tool).relID);
                    tool = sketch.records.get(tool).nextRec;
                    num++;
                }
            }
        }
